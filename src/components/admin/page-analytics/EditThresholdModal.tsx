...
interface Props {
  threshold: PageVisitThreshold
  isOpen: boolean
  onClose: () => void
}

export default function EditThresholdModal({ threshold, isOpen, onClose }: Props) {
  const queryClient = useQueryClient()
  const [formData, setFormData] = useState({
    pageUrl: threshold.pageUrl,
    expectedVisits: threshold.expectedVisits,
    timeWindowHours: threshold.timeWindowHours,
  ...
  const updateMutation = useMutation({
    mutationFn: (data: Partial<typeof formData>) =>
      pageVisitAPI.updateThreshold(threshold.id, data),
    onSuccess: () => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="mt-4 rounded-lg bg-gray-50 p-3 dark:bg-gray-700">
            <p className="text-xs text-gray-600 dark:text-gray-400">
              Total alerts sent: <span className="font-semibold">{threshold.alertsSentCount}</span>
            </p>
            <p className="text-xs text-gray-600 dark:text-gray-400">
              Last alert: <span className="font-semibold">
                {threshold.lastAlertSent ? new Date(threshold.lastAlertSent).toLocaleString() : 'Never'}
              </span>
            </p>
          </div>
