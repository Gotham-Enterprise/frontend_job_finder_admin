...
interface RejectSupervisorModalProps {
  isOpen: boolean;
  fullName: string;
  notes: string;
...
const RejectSupervisorModal: React.FC<RejectSupervisorModalProps> = ({
  isOpen,
  fullName,
  notes,
  onNotesChange,
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl overflow-hidden">
        <div className="px-6 pt-6 pb-4">
          <div className="flex items-start gap-4">
            <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-error-50 dark:bg-error-500/20">
              <svg
        ...
        <div className="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
          ...
          <Button
            onClick={onConfirm}
            className="px-4 py-2 text-sm font-medium text-white bg-error-500 rounded-lg hover:bg-error-600 transition-all duration-200 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            disabled={isDisabled}
          >
            {isLoading ? (
...
export default RejectSupervisorModal;
