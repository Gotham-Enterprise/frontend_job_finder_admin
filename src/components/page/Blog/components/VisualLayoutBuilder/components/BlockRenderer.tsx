...
const BlockRenderer: React.FC<BlockRendererProps> = ({ 
  ...
  const renderColumn = () => {
    ...
    const gap = (block.content as any)?.gap || 16;
    ...
    const renderColumnContent = (column: any) => {
      const { contentType, content } = column;

      switch (contentType) {
        case 'text':
          return (
            <div className="text-sm text-gray-700 leading-relaxed">
              {content.text || 'Column text content...'}
            </div>
          );
        ...
        case 'video':
          ...
          if (isEmbedUrl) {
            return (
              <iframe
                src={embedUrl}
                title={content.title || 'Column video'}
    ...
    return (
      ...
      >
        {columns.slice(0, columnCount).map((column: any, index: number) => (
          <div 
            key={column.id || `col-${index}`}
            className="bg-white border border-gray-200 rounded-lg p-4 min-h-24"
          >
