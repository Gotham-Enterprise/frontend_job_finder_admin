...
const BlogTable: React.FC<BlogTableProps> = ({
  ...
  const getAuthorName = (post: any): string => {
    ...
    if (metadataAuthor) {
      return (
        metadataAuthor.name ||
        `${metadataAuthor.firstName || ""} ${metadataAuthor.lastName || ""}`.trim() ||
        "Unknown Author"
      );
    }
  ...
  const getBlogImage = (post: any): string => {
    const image = post.featuredImage || post.image || (post as any)?.metadata?.image || (post as any)?.thumbnail;
              ...
              <TableRow className="border-b border-gray-200 dark:border-gray-800">
                {canShowCheckboxes && (
                  <TableCell className="py-4 px-6 w-12 font-semibold text-gray-900 dark:text-white">
                    <Checkbox checked={allSelected} onChange={(checked) => onSelectAll(checked)} />
                  </TableCell>
                )}
                  ...
                  >
                    ...
                    <TableCell className="py-4 px-6">
                      <div className="flex items-center flex-wrap gap-1">
                        ...
                        {post.tags?.length > 2 && (
                          <span className="text-xs text-gray-500 dark:text-gray-400">+{post.tags.length - 2} more</span>
                        )}
                      </div>
                    </TableCell>
                    <TableCell className="py-4 px-6">
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-gray-900 dark:text-white">{getAuthorName(post)}</span>
                      </div>
                    </TableCell>
                    <TableCell className="py-4 px-6">
                    ...
                    <TableCell className="py-4 px-6">
                      <p className="text-sm text-gray-900 dark:text-white">{formatDate(post.createdAt)}</p>
                    </TableCell>
