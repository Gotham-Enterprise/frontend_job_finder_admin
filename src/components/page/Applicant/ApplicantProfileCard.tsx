...
const ResumeDisplay = ({ resume, isViewingResume, onViewDocument }: ResumeDisplayProps) => {
    if (!resume) {
        return (
    ...
    return (
        <div 
            ...
            onClick={() => !isViewingResume && onViewDocument(resume.fileUrl, resume.fileObjectKey)}
        >
...
const renderProfileFields = (fields: ProfileFieldConfig[]) => {
    return fields
        ...
        .map((field, index) => (
            <ProfileField 
                key={index} 
                label={field.label} 
                value={field.isNumeric ? `${field.value} Year(s)` : field.value!} 
            />
        ));
};
...
export default function ApplicantProfileCard({ applicant, onViewDocument, isViewingResume = false }: ApplicantProfileCardProps) {
    ...
    const getProfileFields = (applicant: ApplicantData): ProfileFieldConfig[] => [
        { label: 'Experience', value: applicant.yearsOfExperience, isNumeric: true },
        { label: 'Phone', value: applicant.phone },
        { label: 'Email', value: applicant.email },
        { label: 'Location', value: applicant.location },
        { label: 'Joined', value: applicant.joinedDate },
        { label: 'Last Active', value: applicant.lastActiveDate }
    ];
        ...
        <div className="rounded-xl bg-white p-6 shadow-lg border border-gray-100 dark:bg-gray-800 dark:border-gray-700 sm:p-8 mb-6">
            <div className="flex flex-col items-center">
                ...
                <div className="w-full space-y-4 mb-6">
                    {renderProfileFields(getProfileFields(applicant))}
                </div>
