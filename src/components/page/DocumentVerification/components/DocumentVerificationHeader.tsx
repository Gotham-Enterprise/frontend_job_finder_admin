...
interface Props {
  ...
  isSaving: UseDocumentVerificationLogic["isSaving"];
  onFilterChange: UseDocumentVerificationLogic["onFilterChange"];
  onBatchUpdate: UseDocumentVerificationLogic["onBatchUpdate"];
}
...
const DocumentVerificationHeader: FC<Props> = ({ filters, checkedItems, isSaving, onFilterChange, onBatchUpdate }) => {
    ...
    <div className="flex flex-col gap-4 px-6 py-5 border-b border-gray-200 dark:border-gray-800">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex flex-col flex-1 gap-2">
          <h3 className="text-xl font-semibold text-gray-800 dark:text-white">Document Verifications</h3>
          <Tabs className="w-full" value={filters.status} onValueChange={onChangeTab}>
            <TabsList className="mb-2">
              ...
              <CustomTabTrigger value="rejected">Rejected</CustomTabTrigger>
              <CustomTabTrigger value="verified">Verified</CustomTabTrigger>
            </TabsList>
          </Tabs>
        </div>
      </div>
      ...
      {checkedItems.length > 0 && (
        <div className="flex flex-row gap-4">
          <button
            className="text-md text-gray-900 bg-gray-50 p-2 rounded-sm"
            disabled={isSaving}
