...
const CreateUserForm: React.FC<CreateUserFormProps> = ({ onSubmit, onCancel, isLoading = false }) => {
    ...
    if (apiRoles.length > 0) {
      const mappedRoles = apiRoles
        .filter((role) => {
          ...
          return !roleName.includes('super admin') && !roleName.includes('superadmin') && roleName !== 'admin';
        })
        .map((role) => ({
      ...
      const uniqueRoles = mappedRoles.filter(
        (role, index, self) => index === self.findIndex((r) => r.value === role.value)
      );
  ...
  const updatePermission = useCallback(
    (module: string, permissionType: 'add' | 'edit' | 'view' | 'delete', value: boolean) => {
      setFormData((prev) => {
          ...
          <div className="w-80 px-6 py-6 space-y-6 overflow-y-auto">
            ...
            <div className="transform transition-all duration-200">
              ...
              <Select
                ...
                disabled={isLoading || createRoleMutation.isPending}
              />
              {errors.role && <p className="mt-1.5 text-xs text-error-500 animate-pulse">{errors.role}</p>}
            </div>
          </div>
          ...
          <div className="flex-1 border-l border-gray-200 dark:border-gray-700" style={{ backgroundColor: '#f5f8fa' }}>
            <div className="px-6 py-6 space-y-4 overflow-y-auto h-full">
              <div className="py-2" style={{ backgroundColor: '#f5f8fa' }}>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">Access Permissions</h3>
                        ...
                        <div className="flex items-center justify-between p-4 rounded-lg border border-gray-100 dark:border-gray-700 hover:bg-green-50 dark:hover:bg-green-900/10 transition-colors duration-200 min-w-0">
                          ...
                          <div className="flex-shrink-0 ml-3">
                            <CompactSwitch
                              ...
                              disabled={
                                ['jobSeekers', 'applications', 'careers', 'tickets', 'unlockRequest'].includes(
                                  module.key
                                ) ||
                                isLoading ||
                        ...
                        <div className="flex items-center justify-between p-4 rounded-lg border border-gray-100 dark:border-gray-700 hover:bg-yellow-50 dark:hover:bg-yellow-900/10 transition-colors duration-200 min-w-0">
                          ...
                          <div className="flex-shrink-0 ml-3">
                            <CompactSwitch
                              ...
                              disabled={
                                ['jobs', 'applications', 'tickets', 'careers', 'coupons', 'unlockRequest'].includes(
                                  module.key
                                ) ||
                                isLoading ||
                                createRoleMutation.isPending ||
                    ...
                    <button
                      ...
                      onClick={() => {
                        const globalPermissions = dynamicModules.reduce((acc, module) => {
                          acc[module.key] = {
                            view: true,
                            add: !['jobSeekers', 'applications', 'careers', 'tickets', 'unlockRequest'].includes(
                              module.key
                            ),
                            edit: !['jobs', 'applications', 'tickets', 'careers', 'coupons', 'unlockRequest'].includes(
                              module.key
                            ),
                            delete: module.key === 'blog',
                          };
