...
export default function ProfileCard({
  ...
  const getResumesToShow = () => {
    ...
    const resumes = profileData.documents.filter((doc) => isResumeDocument(doc) && doc.url);
  ...
  const getDocumentsToShow = () => {
    ...
    const documents = profileData.documents.filter((doc) => !isResumeDocument(doc) && doc.objectKey);
      ...
      <div className="flex flex-col items-center">
        <div className={`relative ${variant === "compact" ? "mb-4" : "mb-6"} inline-block`}>
          ...
          {showStatusIndicator && profileData.status && (
            <div
              className={`absolute bottom-1 right-1 w-6 h-6 rounded-full border-3 border-white dark:border-gray-800 shadow-sm ${getStatusIndicatorVariant(profileData.status)}`}
            ></div>
          )}
        </div>
        ...
        <div className={`text-center ${variant === "compact" ? "mb-4" : "mb-6"}`}>
          <h2
            className={`${variant === "compact" ? "text-lg" : "text-lg"} font-bold text-gray-900 dark:text-white mb-2`}
          >
            {profileData.name}
          </h2>
          {profileData.email && <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">{profileData.email}</p>}
          <div className="flex items-center justify-center gap-2">
            ...
            {profileData.status && (
              <span
                className={`inline-block px-3 py-1 text-sm font-medium rounded-full ${getProfileStatusBadgeVariant(profileData.status)}`}
              >
                {profileData.status}
              </span>
            )}
        ...
        {showDocuments &&
          ...
          getResumesToShow().length > 0 && (
            <div className="w-full mb-6">
              <div className="flex items-center justify-between mb-4">
                <h4
                  className={`${variant === "compact" ? "text-base" : "text-lg"} font-bold text-gray-900 dark:text-white`}
                >
                  Resume
                </h4>
                {profileData.documents.filter((doc) => isResumeDocument(doc)).length > 3 && (
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/30 px-2 py-1 rounded-full">
                      {profileData.documents.filter((doc) => isResumeDocument(doc)).length} files
                    </span>
                    {!showAllResumes ? (
                      <Button
                        variant="ghost"
              ...
              <div
                className={getContainerClasses(
                  showAllResumes,
                  profileData.documents.filter((doc) => isResumeDocument(doc) && doc.objectKey).length
                )}
              >
                {getResumesToShow().map((document, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-200 truncate capitalize">
                        {document.type}
                      </p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 truncate mt-0.5">{document.fileName}</p>
                    </div>
              ...
              {showAllResumes &&
                profileData.documents.filter((doc) => isResumeDocument(doc) && doc.objectKey).length > 5 && (
              ...
              {!showAllResumes &&
                profileData.documents.filter((doc) => isResumeDocument(doc) && doc.objectKey).length > 3 && (
                  <div className="mt-3 text-center">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Showing 3 of{" "}
                      {profileData.documents.filter((doc) => isResumeDocument(doc) && doc.objectKey).length} resumes
                    </p>
                  </div>
                )}
            </div>
        ...
        {showDocuments && (!profileData.documents || profileData.documents.length === 0) && (
          <div className="w-full">
            <h4
              className={`${variant === "compact" ? "text-base" : "text-lg"} font-bold text-gray-900 dark:text-white mb-4`}
            >
              Documents
            </h4>
