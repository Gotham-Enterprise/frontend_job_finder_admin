...
export const convertLayoutToHtml = (layout: BlogLayout): string => {
  ...
  const createInlineStyles = (styles: any, blockType: string) => {
    ...
    if (styles.margin) {
      cssProperties.push(
        `margin: ${styles.margin.top}px ${styles.margin.right}px ${styles.margin.bottom}px ${styles.margin.left}px !important`
      );
    }
    ...
    if (styles.padding) {
      cssProperties.push(
        `padding: ${styles.padding.top}px ${styles.padding.right}px ${styles.padding.bottom}px ${styles.padding.left}px !important`
      );
    }
    ...
    if (styles.border) {
      ...
      if (border.width && border.width > 0) {
        cssProperties.push(
          `border: ${border.width}px ${border.style || "solid"} ${border.color || "#e5e7eb"} !important`
        );
      }
      if (border.radius) {
    ...
    if (styles.boxShadow) {
      ...
      cssProperties.push(
        `box-shadow: ${shadow.x}px ${shadow.y}px ${shadow.blur}px ${shadow.spread}px ${shadow.color} !important`
      );
    }
  ...
  const createUniqueCss = (styles: any, blockType: string, index: number) => {
    ...
    const cssStyle = cssProperties.length > 0 ? `<style>.${uniqueId} { ${cssProperties.join("; ")} }</style>` : "";
    ...
    .map((block, index) => {
      switch (block.type) {
        case "heading":
          ...
          const headingText =
            headingBlock.content.text === "Your Heading Here" ? "Your Heading Here" : headingBlock.content.text;
        ...
        case "paragraph":
          ...
          const paragraphText =
            paragraphBlock.content.text === "Start writing your content here..."
              ? "Start writing your content here..."
              : paragraphBlock.content.text;
          const { cssClass: paragraphClass, cssStyle: paragraphStyle } = createUniqueCss(
            block.styles,
            "paragraph",
            index
          );
        ...
        case "image":
          ...
          const imageContainerAlign =
            block.styles?.imageAlign === "center"
              ? "text-center"
              : block.styles?.imageAlign === "right"
                ? "text-right"
                : "text-left";
        ...
        case "gallery":
          ...
          const galleryImages = galleryBlock.content.images
            .map(
              (img, imgIndex) =>
        ...
        case "video":
          ...
          const videoContainerAlign =
            block.styles?.videoAlign === "center"
              ? "text-center"
              : block.styles?.videoAlign === "right"
                ? "text-right"
                : "text-left";
