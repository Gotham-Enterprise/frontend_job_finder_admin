...
export const useEmployerLogic = () => {
  ...
  const getInitialFilters = (): EmployerFilters => {
    ...
    if (hasUrlParams) {
      ...
      const isSimpleNavigation = (!urlPage || urlPage === "1") && !urlName && !urlLocation && !urlStatus;
  ...
  const filterChange = useMemo(
    () => (key: keyof EmployerFilters, value: any) => {
      startTransition(() => {
        setFilters((prev) => ({
          ...prev,
  ...
  const viewEmployer = useCallback(
    (employerId: string) => {
      saveScrollPosition();
      saveSearchState();
      if (typeof window !== "undefined") {
  ...
  const hasActiveFilters = useMemo(() => {
    return !!(searchInput || filters.location || filters.status || selectedEmployerId);
    ...
    if (data && !isLoading) {
      ...
      if (hasUrlParams) {
        ...
        import("@/services/utils/autoScroll").then(({ restoreScrollWithItemHighlight }) => {
          restoreScrollWithItemHighlight("employer-selected-item", "employer-scroll-position");
        });
      }
    }
  ...
  useEffect(() => {
    const isOnPageOneWithNoFilters = filters.page === 1 && !filters.name && !filters.location && !filters.status;
